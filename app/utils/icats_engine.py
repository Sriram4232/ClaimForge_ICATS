...
def evaluate_claim(policy, claim, status=None):
    """
    ...
    Supports status-level override if the claim has already been APPROVED.
    """
    ...
    if is_lapsed_state:
        if premiums_paid >= 3:
            ...
            if status != "APPROVED":
                ...
                r6_passed = True
                r6_impact = "NONE"
                msg = "Policy lapsed, but override approved for payment."
    elif grace_applied:
    ...
    if is_suicide:
        if days_since_inception <= 365:
            ...
            if status != "APPROVED":
                ...
                r7_passed = True
                r7_impact = "NONE"
                msg += " (Override approved by assessor)"
        else:
    ...
    if status == "APPROVED":
        ...
        decision_reason = "Assessor manually audited and approved this claim."
    elif risk_score > 70:
